...
neo4j_uri = "bolt://localhost:7688"  # running on docker instance while 7687 busy
neo4j_username = "neo4j"
neo4j_password = "neo4j_password"
driver_neo = GraphDatabase.driver(neo4j_uri, auth=(neo4j_username, neo4j_password))
...
def sql_records_intake_clean(job, server, username, password):
    conn = pymssql.connect(server=server, user=username,
                           password=password)
    cursor = conn.cursor()
    search = job_map_select[job]
    cursor.execute(search)
    records = cursor.fetchall()
    # THE CODE BELOW WORKS WITH PYODBC BUT NOT WITH PYMSSQL
...
def job3(server, username, password, driver_neo4j):
    job = 'job3'
    # running this creates the csv file that copies onto the neo4j import
    # therefore any potential out capture here below is for testing debugging only
    sql_records_intake_clean(job, server, username, password)
    current_directory = os.getcwd()
    source_path = os.path.join(current_directory, f'sql_intake_{job}.csv')
    # adjust if using docker or otherwise
    neo4j_import_path = os.path.join(os.getenv('HOME'), 'neo4j_pii/import')
    destination_path = os.path.join(neo4j_import_path, f'sql_intake_{job}.csv')
    shutil.copy(source_path, destination_path)
    logger.info(f'sql_intake_{job}.csv file copied to neo4j import folder')

    # import time
    # def check_apoc(driver):
    #     with driver.session() as session:
    #         result = session.run("CALL apoc.help('apoc')")
    #         for record in result:
    #             print(record)
    #
    # check_apoc(driver_neo)
    # time.sleep(3)
    ...
    with open(f"smb_import_{job}.cql", "r") as file:
    ...
    #                 session.run(query)
    #
    # with open(r"smb_import_job3_apoc.cql", "r") as file:
    #     cypher_query = file.read()
    #     # Replace the placeholder in the .cql file with the actual CSV file path
    #     cypher_query = cypher_query.replace("<CSV_FILE_PATH>", csv_file_path)
    #     cypher_query_list = cypher_query.split('\n\n')
    #     run_query(cypher_query_list)
...
def getresults(account_id, driver_neo4j):
    with driver_neo4j.session() as session:
        results, duplicate_entries = session.execute_read(fraud_query, account_id)
        return results, duplicate_entries


def log_update() -> List[str]:
